...
class ScenarioFlowTests(TestCase):
    ...
    def test_excel_includes_readme_and_kpi_guidance(self):
        _, _, _, _, xlsx = simulate_two_trucks(
            days=1,
            route_key="SUR",
            Q_proc_m3h=6.0,
            TS_in=0.05,
        ...
        excel_bytes = io.BytesIO(xlsx.read())
        workbook = pd.ExcelFile(excel_bytes)

        self.assertIn("README", workbook.sheet_names)
        kpi_sheet = workbook.parse("KPI")
        self.assertIn("Costo total (CLP)", kpi_sheet.columns)
        readme_sheet = workbook.parse("README", header=None)
        self.assertEqual(readme_sheet.iloc[0, 0], "Cómo usar este archivo")
